...
int state = 0, count = 0;
bool led_states[4];
int stable_count = 0, stable_break = 25; 
...
void check_stable(){
  if(abs(x - set_point_x)<2.0 && abs(y - set_point_y)<2.0){
    stable_count ++;
    }
  else{
    stable_count = 0;
...
void loop() {
  ...
  while(state == 0){
      ...
      if(stable_count > stable_break){
        ...
        Kp_x=1.4; Ki_x=0.00002; Kd_x=500.0; Ks_x=0.5;
        Kp_y=1.5; Ki_y=0.00002; Kd_y=700.0; Ks_y=0.5;
        delay(20);
        }
      else {
          //Calculate the target angles by use of the PID 
          output_x = computePID_x(x);
          output_y = computePID_y(y);
          Kp_x=1.4; Ki_x=0.0; Kd_x=900.0; Ks_x=0.4;
          Kp_y=1.5; Ki_y=0.0; Kd_y=1100.0; Ks_y=0.4;
          delay(70);
          }
      //Move servos
